...
typedef struct{
    ...
    Fish *bucket[BUCKET_SIZE]; //a bucket that can hold at most BUCKET_SIZE number of fish pointers
    unsigned char numFish; //number of fish in the bucket
} Fisher;

typedef struct{
    Fish *fish[POND_CAPACITY]; //an array that can hold at most BUCKET_SIZE number of fish pointers
...
int addFish(Pond *p, unsigned char size, char *species){
    ...
    p->fish[pSize] = malloc(sizeof(Fish)); //allocate memory first
...
char likes(Fisher *fisher, Fish *f){
    if (((int)f->size >= (int)fisher->keepSize) && f->species!="Sunfish"){  //if the size of the fish is >= the keepsize of the fish is not a Sunfish then return 1, otherwise return 0
        return 1;
    }
    return 0;
}
...
int keep(Fisher *fisher, Fish *f){
    ...
    if (pSize == BUCKET_SIZE){
        return 0; //if the bucket is at capacity then return 0
    }
    //if the bucket is not at capacity then add a fish to it by setting the fish's attributes
    fisher->bucket[pSize] = f;
    pSize++;
    fisher->numFish = (char) pSize;
    return 1;
}
//simulate casting a lines
int castLine(Fisher *fisher, Pond *p){
    ...
    if (l){ //see if the fisher likes fish at the random index
        ...
        if (k){ //see if the fisher wants to keep it
            ...
            if (rIndex != nF-1){ 
                p->fish[rIndex] = p->fish[nF-1]; //if the random index is not the last index, set the value at the random index to the value at the last index
            }
            nF--;
            p->numFish = (char) nF;
        }   
    ...
    for (int i=0; i<nF1; i++){
        if (likes(fisher2, &(*(fisher1->bucket[i])))){ //see if the fisher likes fish 
            if (keep(fisher2, &(*(fisher1->bucket[i]))) == 0){ //see if the fisher wants to keep it
                if (pF!=POND_CAPACITY){ //if not then add the fish to the pond
                    p->fish[pF] = fisher1->bucket[i];
                    pF++;
                    p->numFish = (char)pF;
                }
            }
